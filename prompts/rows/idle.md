Create a single horizontal sprite strip for the Codex app digital pet `miko` in the state `idle`.
...
Repair attempt 1:
- The previous `idle` strip failed QA: idle used extraction method slots; regenerate the row or inspect slot slicing

Create a single horizontal sprite strip for the Codex app digital pet `miko` in the state `waiting`.
...
Animation action: sleeping-on-the-ground waiting loop. Show Miko lying down on the ground asleep in every frame, curled or gently stretched in a compact chibi pose that fits fully inside each 192x208 slot. Use subtle breathing, tiny ear twitch, closed sleepy eyes, and a small head/body bob across the 6 frames. Add one tiny translucent pale-blue/white nasal sleep bubble attached directly to Miko's nose or mouth in every frame; the bubble must visibly change size across the loop, for example tiny, medium, large, medium, tiny, almost gone. The nasal bubble is allowed only because it is attached to the face and state-relevant; it must not float away, detach, cross slots, contain text, or become a separate large object. The red gohei/banner wand remains part of the identity and must stay attached/held close or tucked beside the body inside each frame slot, never crossing into a neighbor slot. Do not add bedding, pillows, scenery, floor marks, shadows, dust, dream bubbles, Z text, floating symbols, or detached effects other than the attached nasal bubble.
...
Repair attempt 1:
- The previous `waiting` strip failed QA: User rejected the waiting strip because every two frames show overlapping pet or prop parts; subagent QA also warned the background is not exact pure #FF00FF.
...
Repair attempt 2:
- The previous `waiting` strip failed QA: User requested changing the waiting animation to Miko lying on the ground asleep; regenerate the waiting row only.
- Regenerate the entire row, not just one pose.
- Fill every requested frame slot with one complete centered full-body pet pose.
- Keep large gaps of pure chroma key only between slots; do not leave a requested slot empty.
- Avoid pose overlap, clipping, edge slivers, extra partial sprites, and detached fragments from neighboring poses.
...
Repair attempt 3:
- The previous `waiting` strip failed QA: User requested adding a changing nasal sleep bubble to the lying-down sleeping waiting animation; regenerate the waiting row only.
- Regenerate the entire row, not just one pose.
- Fill every requested frame slot with one complete centered full-body pet pose.
- Keep large gaps of pure chroma key only between slots; do not leave a requested slot empty.
- Avoid pose overlap, clipping, edge slivers, extra partial sprites, and detached fragments from neighboring poses.

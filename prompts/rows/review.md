Create a single horizontal sprite strip for the Codex app digital pet `miko` in the state `review`.
...
Repair attempt 1:
- The previous `review` strip failed QA: review used extraction method slots; regenerate the row or inspect slot slicing

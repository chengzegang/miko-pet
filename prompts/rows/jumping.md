Create a single horizontal sprite strip for the Codex app digital pet `miko` in the state `jumping`.
...
Animation action: dancing spin jump while holding the gohei. Show a 5-frame loop where Miko holds the red gohei/banner wand in one hand and twirls/dances through the jump: frame 1 crouched dance preparation with gohei held close, frame 2 rising while turning clockwise, frame 3 airborne spin peak with body rotated and gohei raised, frame 4 descending while continuing the turn, frame 5 soft dance landing/settle ready to loop. Keep the gohei in the hand in every frame; do not drop it, place it on the ground, swap it between hands, or hold it with both hands. Express rotation through body orientation, hair sweep, sleeve/limb pose, and gohei angle only. Do not draw spin rings, swirl marks, motion arcs, sparkles, dust, shadows, floor marks, scenery, or detached effects.


State-specific requirements:
- Show the jump and spin through pose, body orientation, gohei angle, hair sweep, and vertical body position only: dance preparation, rising turn, airborne spin peak, descending turn, soft settle.
...
Repair attempt 1:
- The previous `jumping` strip failed QA: User requested changing jumping to Miko holding the gohei while spinning and dancing; regenerate only jumping.

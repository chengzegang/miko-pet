Create a single horizontal sprite strip for the Codex app digital pet `miko` in the state `waving`.
...
- Show the greeting by waving Miko's handheld red ritual wand/gohei/banner, not by waving an empty hand.
- Keep the red wand attached to the hand in every frame, with its white zigzag paper streamers and small red slips moving as a single compact prop.
- Use a compact wand motion cycle: wand low, wand raised, wand tilted, wand returning.
- The wand and paper streamers must stay fully inside each frame slot and must not cross into neighboring slots.
- Do not draw wave marks, motion arcs, lines, sparkles, symbols, or floating effects around the wand or hand.
